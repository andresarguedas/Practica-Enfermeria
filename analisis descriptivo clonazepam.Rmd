...
x <- c("ggplot2", "scales", "gridExtra", "grid", "magrittr", "plyr", "cowplot", "lubridate")
...
pie <- pie + scale_fill_manual(values = c("brown2", "blue3"), name = "Sexo") +  blank_theme +
  ...
  geom_text(aes(y = c(cumsum(porcentaje)[-length(porcentaje)] + .5, cumsum(porcentaje)[-length(porcentaje)]), 
            label = percent(porcentaje)), size=5) + ggtitle(paste("Figura ", i, ":\nPorcentaje del total de prescripciones de clonazepam por sexo", sep = ""))

g1 <- arrangeGrob(pie, bottom = textGrob("Fuente: Datos de consumo de psicotrópicos de la CCSS", x = 0, hjust = -0.1, vjust = -0.1, gp = gpar(fontsize = 10)))

grid.draw(g1)
```

  De la Figura `r i` se puede observar que la mayor cantidad de prescripciones son dadas a mujeres, las cuales representan un 64.84% del total de pacientes, por lo que hay congruencia entre la cantidad de mujeres y la cantidad de prescripciones recetadas a mujeres. Esto parece indicar que, por lo menos en términos del promedio de prescripciones entre hombres y mujeres, no van a haber diferencias tan notables entre hombres y mujeres, relativamente.

```{r Prescripciones por edad, echo = F, fig.pos = 'p'}
...
barras <- ggplot(data = clon.edad, aes(x = Edad, y = x)) + 
  geom_col(fill = "brown") + 
  ylab("Prescripciones") +
  ggtitle(paste("Figura ", i, ":\nCantidad de prescripciones por grupos de edad", sep = "")) +
  theme1 +
  theme(axis.text.x = element_text(angle = 70, vjust = 0.5))

g3 <- arrangeGrob(barras, bottom = textGrob("Fuente: Datos de consumo de psicotrópicos de la CCSS", x = 0, hjust = -0.1, vjust = -0.1, gp = gpar(fontsize = 10)))
...
```{r Piramide, echo = F, warning = F, fig.pos = 'p'}
...
piramide <- ggplot(clon.sexoedad, aes(x = Edad, y = Prescripciones, fill = Sexo)) + 
                   ...
                   xlab("Edad") +
                   ylab("Porcentaje") +
                   ggtitle(paste("Figura ", i, ":\nPorcentaje del total de prescripciones por grupos de edad y sexo", sep = ""))
                   

g2 <- arrangeGrob(piramide, bottom = textGrob("Fuente: Datos de consumo de psicotrópicos de la CCSS", x = 0, hjust = -0.1, vjust = -0.1, gp = gpar(fontsize = 10)))

grid.draw(g2)
```

  Al separar tanto por sexo como por grupos etarios, se puede ver, de la Figura `r i`, que aunque la distribución porcentual de las prescripciones es parecida tanto para hombres como para mujeres, en el caso de las mujeres parece haber un porcentaje mayor de prescripciones para personas entre los 46 y 65 años, lo que es consistente con lo observado en la Figura `r i - 1`. Es importante señalar que en la Figura `r i` si usó un porcentaje por sexo dado que, por lo visto en la Figura `r i - 2`, hay una cantidad considerablemente mayor de prescripciones a mujeres que hombres, por lo que si se separan los datos absolutos se vuelve difícil ver la distribución de estos, lo que dificulta el análisis.

```{r Media por edad, echo = F, message = F, fig.pso = 'p'}
i <- i + 1
...
serie.tiempo <- ggplot(data = clon.tiempo, aes(x = tiempo, y = x)) + 
  ...
  xlab("Año") + 
  theme1 +
  ggtitle(paste("Figura ", i, ":\nCantidad de prescripciones de clonazepam por mes y año", sep = ""))

g5 <- arrangeGrob(serie.tiempo, bottom = textGrob("Fuente: Datos de consumo de psicotrópicos de la CCSS", x = 0, hjust = -0.1, vjust = -0.1, gp = gpar(fontsize = 10)))
...
barras.ddd <- ggplot(data = clon.ddd, aes(x = edad.rec, y = mg)) +
                     ...
                     ylab("Prescripciones") +
                     xlab("Edad") +
                     ggtitle(paste("Figura ", i, ":\nCantidad de prescripciones por encima de la DDD\n por grupos de edad", sep = "")) +
                     theme1 +
                     theme(axis.text.x = element_text(angle = 70, vjust = 0.5))

g8 <- arrangeGrob(barras.ddd, bottom = textGrob("Fuente: Datos de consumo de psicotrópicos de la CCSS", x = 0, hjust = -0.1, vjust = -0.1, gp = gpar(fontsize = 10)))
...
dispersion <- ggplot(data = clon.ddd, aes(x = clon.ddd$mg, y = clon.ddd$prescripciones)) +
              ...
              ylab("Cantidad de prescripciones") +
              ggtitle(paste("Figura ", i, ":\nCantidad de prescripciones totales y mayores a la DDD por paciente\n(La línea roja representa la línea identidad)", sep = ""))

g9 <- arrangeGrob(dispersion, bottom = textGrob("Fuente: Datos de consumo de psicotrópicos de la CCSS", x = 0, hjust = -0.1, vjust = -0.1, gp = gpar(fontsize = 10)))
...
for(l in 1:length(u1)) {
  v1[l, 1] <- sum(clon.medico$x > u1[l]) / 8405 * 100
  v1[l, 2] <- sum(clon.medico$x[clon.medico$x > u1[l]]) / 701795 * 100
}
...
medicos.por <- ggplot(data = v1, aes(x = V1, y = V2)) + 
               ...
               theme1 +
               ggtitle(paste("Figura ", i, ":\nPorcentaje de prescripciones emitidas por\n un cierto porcentaje de médicos", sep = ""))

g6 <- arrangeGrob(medicos.por, bottom = textGrob("Fuente: Datos de consumo de psicotrópicos de la CCSS", x = 0, hjust = -0.1, vjust = -0.1, gp = gpar(fontsize = 10)))

grid.draw(g6)
```

  Con base en la Figura `r i`, se puede notar que un porcentaje bastante pequeño de los médicos receta la gran mayoría de las recetas de clonazepam. Por ejemplo, un `r round(v1[10,1], 0)`% de los médicos recetan el `r round(v1[10, 2], 2)`% del total de prescripciones, mientras que un `r round(v1[52, 1], 0)`% de los médicos recetan un `r round(v1[52, 2], 2)`%. De igual forma es importante resaltar que un `r round(v1[403, 2], 2)`% de todas las prescripciones son recetadas por un `r round(v1[403, 1], 2)`% de los médicos, aproximadamente `r round(nrow(clon.medico) * v1[403, 1] / 100, 0)` médicos. Lo anterior demuestra que hay una cantidad bastante pequeña de médicos que dan la mayoría de prescripciones de clonazepam del país.

```{r Medicos con DDD, echo = F}
i <- i + 1
...
dispersion.m <- ggplot(data = clon.medico, aes(x = clon.medico$ddd, y = clon.medico$x)) +
              ...
              xlab("Cantidad de prescripciones mayores a la DDD") +
              ylab("Cantidad de prescripciones") +
              ggtitle(paste("Figura ", i, ":\nCantidad de prescripciones total y mayores\n a la DDD recetadas según médico", sep = ""))

g7 <- arrangeGrob(dispersion.m, bottom = textGrob("Fuente: Datos de consumo de psicotrópicos de la CCSS", x = 0, hjust = -0.1, vjust = -0.1, gp = gpar(fontsize = 10)))

grid.draw(g7)
```

  Es importante resaltar que un `r round(sum(clon.medico$ddd > 1) / nrow(clon.medico) * 100, 2)`% de los médicos hicieron al menos una prescripción con una dosis mayor a la DDD. Ahora, con respecto a la Figura `r i`, podemos notar que hay un caso extremo que ha hecho más de 400 prescripciones mayores a la DDD, pero, sin contar el caso anterior, todos los médicos han recetado menos de 100 prescripciones con una dosis mayor a la DDD. También vale la pena resaltar que, comparando los resultados de la anterior figura con los de la Figura `r i - 1`, hay una cantidad bastante considerable de médicos que han dado más de 2500 recetas de clonazepam en el período de estudio. Lo anterior equivale a al menos `r round(2500 / 60, 2)` recetas de clonazepam, en promedio, por mes, en otras palabras, más de una receta de clonazepam diaria, en promedio. En el caso del médico que recetó cerca de 12000 prescripciones, esto equivale a seis y media recetas de clonazepam diarias, en promedio.

# Especialidad

```{r Especialidad, echo = F}
clon.na$especialidad %<>% gsub("MEDICINA", "", .)
clon.na$especialidad %<>% gsub("FONIATRIAA", "AUDIOLOGIA", .)
clon.na$especialidad %<>% gsub("PSIQUIAT$", "PSIQUIATRIA", .)
clon.na$especialidad %<>% gsub("NEUROLOG$", "NEUROLOGIA", .)
clon.na$especialidad %<>% gsub("PSICOSOMAT$", "PSICOSOMATICA", .)
clon.na$especialidad %<>% gsub("PSICOSOMATI$", "PSICOSOMATICA", .)
clon.na$especialidad %<>% gsub("INFECTOLOGI$", "INFECTOLOGIA", .)
clon.na$especialidad %<>% gsub("GASTROENTER$", "GASTROENTEROLOGIA", .)
clon.na$especialidad %<>% gsub("NEUROCARDI$", "NEUROCARDIO", .)
clon.na$especialidad %<>% gsub("GERIATRI$", "GERIATRIA", .)
clon.na$especialidad %<>% gsub("CARDIOLO$", "CARDIOLOGIA", .)
clon.na$especialidad %<>% gsub("REHABILITAC$", "REHABILITACION", .)
clon.na$especialidad %<>% gsub("REUMATOLOGI$", "REUMATOLOGIA", .)
clon.na$especialidad %<>% gsub("DERMATOLOGI$", "DERMATOLOGIA", .)
clon.na$especialidad %<>% gsub("ENDOCRINOLO$", "ENDOCRINOLOGIA", .)
clon.na$especialidad %<>% gsub("ADOLESCENTE$", "ADOLESCENTES", .)
clon.na$especialidad %<>% gsub("EMERGENC$", "EMERGENCIA", .)
clon.na$especialidad %<>% gsub("FA$", "FAMILIAR", .)
clon.na$especialidad %<>% gsub("UCI MEDICOS", "", .)
clon.na$especialidad %<>% gsub("UCI", "", .)
clon.na$especialidad %<>% gsub("PENSION", "", .)
clon.na$especialidad %<>% gsub("(PSIQUIATRIA).*", "\\1", .)
clon.na$especialidad %<>% gsub("(PEDIATRIA).*", "\\1", .)
clon.na$especialidad %<>% gsub("(PSICOLOGIA).*", "\\1", .)
clon.na$especialidad %<>% gsub("(CIRUGIA).*", "\\1", .)
clon.na$especialidad %<>% gsub("(ODONTOLOGIA).*", "\\1", .)
clon.na$especialidad %<>% gsub("(RADIOLOGIA).*", "\\1", .)
clon.na$especialidad %<>% gsub("(GINECO).*", "\\1", .)
clon.na$especialidad %<>% gsub("(ONCOLOGIA).*", "\\1", .)
clon.na$especialidad %<>% gsub("(NUTRICION).*", "\\1", .)
clon.na$especialidad %<>% gsub("(NEUROCARDIO).*", "\\1", .)
clon.na$especialidad %<>% gsub("(FAMILIAR).*", "\\1", .)
clon.na$especialidad %<>% gsub("(CLINICA).*", "\\1", .)
clon.na$especialidad %<>% gsub("(AUDIOLOGIA).*", "\\1", .)
clon.na$especialidad %<>% gsub("NO APLICA", "", .)
clon.na$especialidad %<>% gsub("^\\s+", "", .)
